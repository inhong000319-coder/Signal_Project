...
@Service
public class DeliveredMarkService implements DeliveredMarkUseCase {
    ...
    @Override
    @Transactional
    public DeliveredMarkResult markDelivered(DeliveredMarkCommand command) {
        validate(command);
        ...
        if (message.getSenderUserId().equals(command.getUserId())) {
            throw new BusinessException(ErrorCode.FORBIDDEN, "sender cannot mark delivered");
        }
        ...
        if (existing == null) {
            messageStateRepository.save(MessageState.restore(
                command.getMessageId(), command.getUserId(), MessageStateType.DELIVERED, now
            ));
        } else if (existing.getState() == MessageStateType.SENT) {
            messageStateRepository.updateState(command.getMessageId(), command.getUserId(), MessageStateType.DELIVERED, now);
        }

        boolean updated = syncCursorRepository.updateLastDelivered(command.getConversationId(), command.getUserId(), command.getMessageId());
        if (!updated) {
        ...
        return new DeliveredMarkResult(command.getConversationId(), command.getUserId(), command.getMessageId());
    }

    private void validate(DeliveredMarkCommand command) {
